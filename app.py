...
class ContraRSTApp(toga.App):
    ...
    def startup(self):
        ...
        self.main_window = toga.MainWindow(
            title="ContraRST - Sistema de Gestión Contable",
            size=(1300, 800),
            resizable=True,
        )
        ...
        self.main_box = toga.Box(style=Pack(direction=ROW, flex=1))
        ...
        self.content_box = toga.Box(style=Pack(direction=COLUMN, flex=1, margin=20))
    ...
    def create_sidebar(self):
        ...
        logo.add(
            toga.Label("📊 ContraRST", style=Pack(font_size=22, font_weight="bold", color=COLORS['primary'])),
            toga.Label("Régimen Simple", style=Pack(font_size=12, color=COLORS['gray_600']))
        )
        sidebar.add(logo)
        ...
        menu_items = [
            ...
            ("📒", "Libro Diario", self.show_libro_diario),
            ("📈", "Libro Mayor", self.show_libro_mayor),
            ("📄", "Conciliación", self.show_conciliacion),
            ("🧾", "RST · IVA", self.show_rst_iva),
            ("📎", "Reportes PDF", self.show_reportes),
            ("📋", "PUC", self.show_puc),
        ]
        ...
        sidebar.add(
            toga.Button(
                ...
                style=Pack(margin=10, background_color=COLORS['transparent'], text_align="left")
            )
        )
        sidebar.add(
            toga.Button(
